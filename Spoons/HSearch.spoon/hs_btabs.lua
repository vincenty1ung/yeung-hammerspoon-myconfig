...
obj.overview = { text = "Type t ⇥ to search safari/chrome Tabs.", image = hs.image.imageFromPath(obj.spoonPath ..
"/resources/tabs.png"), keyword = "t" }
    ...
    if #safari_running > 0 then
        ...
        if stat then
            chooser_data = hs.fnutils.imap(data, function(item)
                return { text = item[1], subText = item[2], image = hs.image.imageFromPath(obj.spoonPath ..
                "/resources/safari.png"), output = "safari", arg = item[2] }
            end)
        end
    end
        ...
        if stat then
            for idx, val in pairs(data) do
                ...
                table.insert(chooser_data,
                    { text = val[1], subText = val[2], image = hs.image.imageFromPath(obj.spoonPath ..
                    "/resources/chrome.png"), output = "chrome", arg = val[2] })
            end
        end
    end
...
obj.description = { text = "Browser Tabs Search", subText =
"Search and select one item to open in corresponding browser.", image = hs.image.imageFromPath(obj.spoonPath ..
"/resources/tabs.png") }
