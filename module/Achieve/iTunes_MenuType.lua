...
iTunes.saveartwork = function()
    ...
    if string.len(kind) > 0 then --若为本地曲目
        ...
        [[tell application "iTunes" to get format of current track's artwork 1 as string]])
        ...
        hs.http.encodeForQuery(iTunes.album() .. " " .. iTunes.artist()) ..
        "&country=jp&entity=album&limit=1&output=json"
...
function setmenu()
    if iTunes.state() ~= "stopped" then
        ...
        for i = 7, #(library) do
            table.insert(iTunesBarMenu,
                { title = library[i], fn = function() iTunes.shuffleplay("\"" .. library[i] .. "\"") end })
        end
