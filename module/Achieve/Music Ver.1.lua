...
Music.kind = function()
    ...
    local _, cloudstatus, _ = hs.osascript.applescript(
    [[tell application "Music" to get cloud status of current track as string]])
...
Music.checkrunning = function()
    local _, isrunning, _ = hs.osascript.applescript(
...
Music.saveartwork = function()
    ...
    if Music.kind() == "localmusic" or Music.kind() == "matched" then --若为本地曲目
        ...
        -- 获取图片后缀名
        local _, format, _ = hs.osascript.applescript(
        [[tell application "Music" to get format of artwork 1 of current track as string]])
        if format == nil then
            ...
            artwork = hs.image.imageFromPath(hs.configdir .. "/currentartwork." .. ext):setSize({ h = 300, w = 300 },
                absolute == true)
        end
    ...
    elseif Music.kind() == "applemusic" then -- 若为Apple Music
        if Music.album() ~= songalbum then
            songalbum = Music.album()
            local amurl = "https://itunes.apple.com/search?term=" ..
            hs.http.encodeForQuery(Music.album()) .. "&country=jp&entity=album&limit=10&output=json"
                    ...
                    if songdata.resultCount ~= 0 then
                        ...
                        repeat
                            if songdata.results[i].artistName == Music.artist() then
                                ...
                                artworkfile = hs.image.imageFromURL(artworkurl):setSize({ h = 300, w = 300 },
                                    absolute == true)
...
function setmaincanvas()
    ...
    -- 生成桌面悬浮菜单
    delete(c_menu)
    c_menu = c.new({ x = barframe.x, y = barframe.h + 5, h = artworksize.h + border.y * 2, w = smallsize }):level(c
    .windowLevels.cursor)
...
function setlovedcanvas()
    ...
    if Music.disliked() == true then
        ...
        dislikedimage = hs.image.imageFromPath(hs.configdir .. "/image/notdisliked.png"):setSize(imagesize,
            absolute == true)
    end
    -- 生成菜单
    c_lovedmenu = c.new({ x = menuframe.x + border.x, y = menuframe.y + border.y + artworksize.h * 4 / 5, h = artworksize
    .h / 5, w = artworksize.w }):level(c_menu:level() + 1)
...
function setratecanvas()
    ...
    if Music.kind() == "localmusic" then -- 若为本地音乐
        c_ratemenu = c.new({ x = menuframe.x + border.x + artworksize.w + gap.x, y = menuframe.y + border.y + infosize.h, h =
        artworksize.h / 5, w = c_menu["info"].frame.w }):level(c_menu:level() + 1)
        c_ratemenu:replaceElements(rate)
        local ratemenuframe = c_ratemenu:minimumTextSize(1, c_ratemenu["rate"].text)
        c_ratemenu:frame({ x = menuframe.x + border.x + artworksize.w + gap.x, y = menuframe.y + border.y + infosize.h, h =
        ratemenuframe.h, w = ratemenuframe.w })
    elseif Music.kind() == "matched" then -- 若为已匹配歌曲
        ...
        ratemenuframe.h, w = ratemenuframe.w })
        -- Apple Music Logo
        c_logomenu = c.new({ x = menuframe.x + border.x + artworksize.w + gap.x + ratemenuframe.w + gap.x, y = menuframe
        .y + border.y + infosize.h, h = artworksize.h / 5, w = c_menu["info"].frame.w }):level(c_menu:level() + 1)
        applemusicmark = {
            id = "applemusic",
            frame = { x = 0, y = 1, h = ratemenuframe.h, w = ratemenuframe.w },
            type = "image",
            image = hs.image.imageFromPath(hs.configdir .. "/image/AppleMusicLogo.png"):setSize(
            { h = ratemenuframe.h / 2, w = ratemenuframe.w }, absolute == true),
            imageScaling = "shrinkToFit",
            imageAlignment = "left"
        }
...
function setprogresscanvas()
    ...
    border.y * (1 - per) / 2, h = border.y * per, w = menuframe.w - border.x * 2 }):level(c_menu:level() + 1)
    progressElement = {
        id = "progress",
        type = "rectangle",
