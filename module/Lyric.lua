...
Lyric.api = function(api)
    ...
    if api == "163" then
        ...
        lyricheaders = nil
        function gettrackResult(musicinfo)
            return musicinfo.result
        end

        function gettrackList(trackResult)
        ...
        if musicStatus == 200 then
            ...
            if not songID then
                ...
                if trackList and #trackList > 0 then
                    table.insert(songsResult, trackList)
...
Lyric.setcanvas = function()
    if not c_lyric then
        c_lyric = c.new({ x = 0, y = desktopFrame.h + menubarHeight - 50, h = 50, w = screenFrame.w }):level(c
        .windowLevels.cursor)
    ...
    if songs then
        ...
        for s = 1, #songs, 1 do
            ...
            for i = 1, #songs[s], 1 do
                item = {
                    title = trackName(i) .. " - " .. trackArtist(i),
                    fn = function()
                        songID = trackID(i)
